...
class Trainer(object):
    ...
    def _train_one_epoch(self, epoch):
        # start training
        self.model.train()
        for _, (patches, (label, features)) in enumerate(self.train_dataloader):
            ...
            outputs_NSS = self.model(patches)[1]
            loss_NSS = self.loss_fn(outputs_NSS, features)
            loss_q = self.loss_fn(outputs_q, label)
            self.loss = loss_NSS + loss_q
            self.loss.backward()
            self.optimizer.step()
