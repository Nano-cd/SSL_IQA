...
import model_hub.common as common
...
class Mbcnn2(nn.Module):
    def __init__(self):
        super(Mbcnn2, self).__init__()
...
class Mbcnn(nn.Module):
    def __init__(self, ker_size=3, n_kers=64, n1_nodes=800, n2_nodes=800, conv=common.default_conv, num_classes=5):
        ...
        self.pyconv1 = PyConv2d(in_channels=16, out_channels=[16, 16, 32], pyconv_kernels=[3, 5, 7],
                                pyconv_groups=[1, 4, 8])
        self.pyconv2 = PyConv2d(in_channels=64, out_channels=[16, 16, 32], pyconv_kernels=[3, 5, 7],
                                pyconv_groups=[1, 4, 8])
        self.fc1 = nn.Linear(47104, n1_nodes)
        self.fc2 = nn.Linear(n1_nodes, n2_nodes)
        self.fc3 = nn.Linear(n2_nodes, 5)
        self.fc4 = nn.Linear(11782, n1_nodes)
        self.fc5 = nn.Linear(n1_nodes, n2_nodes)
        self.fc6 = nn.Linear(n2_nodes, 1)

    def forward(self, x):
        # batch_size = am[0].size(0)
        am = x[0].view(-1, 1, x[0].size(-2), x[0].size(-1)).cuda()  # 32×32图片
        al = x[1].view(-1, 1, x[1].size(-2), x[1].size(-1)).cuda()   # 32×32高频梯度图像，
        ai = x[2].view(-1, 1, x[2].size(-2), x[2].size(-1)).cuda()   # 距离信息
        ae = x[3].view(-1, 1, x[3].size(-2), x[3].size(-1)).cuda()
        ...
        h = F.relu(self.fc2(h))
        qi = self.fc3(h)
        qi = F.softmax(qi, dim=1)

        hhh2 = hhh2.to(torch.float32)
