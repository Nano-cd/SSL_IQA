...
def load_pretrained_network(net, model_path, strict=False, weight_keys=None):
    print(f'Loading pretrained model {net.__class__.__name__} from {model_path}')
    state_dict = torch.load(model_path)
    state_dict.pop('head.8.weight')
    state_dict.pop('head.8.bias')
    if weight_keys:
...
class PAQ2PIQ(nn.Module):
    ...
    def __init__(self, num_classes, pretrained):
        ...
        if pretrained:
            load_pretrained_network(self, './checkpoint/P2P_RoIPoolModel-fit.10.bs.120-ca69882e.pth')
    ...
    def forward(self, x):
        ...
        return preds.view(batch_size, -1), feats
